    ...
    public partial class server : Form
    {
        ...
        public void AddMedicine_Click(object sender, EventArgs e)//הוספת תרופה לבסיס נתונים
        {
            try
            {
                ...
                SqlDataAdapter da_medicine = new SqlDataAdapter("SELECT * FROM Medicines", connection);
                SqlCommandBuilder cmd_medicine = new SqlCommandBuilder(da_medicine);
                da_medicine.Fill(ds_medicine, "Medicines");
                DataRow dr_medicine = ds_medicine.Tables["Medicines"].NewRow();
                ...
                ds_medicine.Tables["Medicines"].Rows.Add(dr_medicine);
                da_medicine.Update(ds_medicine, "Medicines");
        ...
        private void delete_m_Click(object sender, EventArgs e)//מחיקת תרופה לפי שם מותג
        {
            try
            {
                ...
                SqlDataAdapter da_medicine_delete = new SqlDataAdapter("SELECT BrandName FROM Medicines", connection);
                da_medicine_delete.Fill(ds_medicine_delete, "Medicines");
                DataRow[] arr_M = ds_medicine_delete.Tables["Medicines"].Select().Where(brand => ((string)brand[0]).Trim().Equals(Medicine_brand_text.Text)).ToArray();
                ...
                else
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        string str_sql = "delete from Medicines where BrandName = @name";
                        connection.Open();
